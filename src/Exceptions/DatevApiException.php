...
/**
 * Exception für DATEV API Fehler
 *
 * @see https://developer.datev.de/
 */
class DatevApiException extends Exception
{
    ...
    public static function fromResponse(int $httpStatusCode, ?array $responseData = null): self
    {
        $message = $responseData['error_description']
            ...
            ?? 'Unbekannter Fehler';
        $errorCode = isset($responseData['error']) && is_string($responseData['error'])
            ? $responseData['error']
            : (isset($responseData['error']['code']) ? (string) $responseData['error']['code'] : null);

        return new self($message, $httpStatusCode, $errorCode, $responseData);
    }

    public static function unauthorized(string $message = 'Ungültiger DATEV Token.'): self
    {
    ...
    {
        return new self(
            'Verbindungsfehler zur DATEV API: ' . $message,
            503,
            'CONNECTION_ERROR'
        );
    }
    ...
    {
        return new self(
            'Keine DATEV-Verbindung für diesen Benutzer konfiguriert.',
            401,
            'NO_CONNECTION'
        );
    }
    ...
    {
        return new self(
            'Ungültige Antwort von der DATEV API.' . ($detail ? ' ' . $detail : ''),
            500,
            'INVALID_RESPONSE'
        );
    }
