...
/**
 * Exception für RingCentral API Fehler
 *
 * @see https://developers.ringcentral.com/api-reference
 */
class RingCentralApiException extends Exception
{
    ...
    public static function fromResponse(int $httpStatusCode, ?array $responseData = null): self
    {
        $message = $responseData['message']
            ...
            ?? 'Unbekannter Fehler';
        $errorCode = isset($responseData['errorCode']) ? (string) $responseData['errorCode'] : null;

        return new self($message, $httpStatusCode, $errorCode, $responseData);
    }

    public static function unauthorized(string $message = 'Ungültiger RingCentral Token.'): self
    {
    ...
    {
        return new self(
            'Verbindungsfehler zur RingCentral API: ' . $message,
            503,
            'CONNECTION_ERROR'
        );
    }
    ...
    {
        return new self(
            'Keine RingCentral-Verbindung für diesen Benutzer konfiguriert.',
            401,
            'NO_CONNECTION'
        );
    }
    ...
    {
        return new self(
            'Ungültige Antwort von der RingCentral API.' . ($detail ? ' ' . $detail : ''),
            500,
            'INVALID_RESPONSE'
        );
    }
