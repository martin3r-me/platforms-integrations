...
/**
 * Exception für Moss API Fehler
 *
 * Moss Public API
 * @see https://public-api.getmoss.com
 */
class MossApiException extends Exception
{
    ...
    {
        $message = $responseData['message'] ?? $responseData['error'] ?? self::HTTP_STATUS_MESSAGES[$httpStatusCode] ?? 'Unbekannter Fehler';
        $errorCode = isset($responseData['code']) ? (string) $responseData['code'] : null;

        return new self($message, $httpStatusCode, $errorCode, $responseData);
    }

    public static function unauthorized(string $message = 'Ungültige Moss Credentials (Client ID/Client Secret).'): self
    {
    ...
    {
        return new self(
            'Verbindungsfehler zur Moss API: ' . $message,
            503,
            'CONNECTION_ERROR'
        );
    }
    ...
    {
        return new self(
            'Keine Moss-Verbindung für diesen Benutzer konfiguriert.',
            401,
            'NO_CONNECTION'
        );
    }
    ...
    {
        return new self(
            'Ungültige Antwort von der Moss API.' . ($detail ? ' ' . $detail : ''),
            500,
            'INVALID_RESPONSE'
        );
    }
