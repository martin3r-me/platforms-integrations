...
/**
 * Exception für Plausible Analytics API Fehler
 *
...
class PlausibleApiException extends Exception
{
    ...
    public const HTTP_STATUS_MESSAGES = [
        ...
        401 => 'Unauthorized - Authentifizierung erforderlich oder API-Key ungültig.',
    ...
    public static function fromResponse(int $httpStatusCode, ?array $responseData = null): self
    {
        $message = $responseData['error']
            ...
            ?? 'Unbekannter Fehler';
        $errorCode = isset($responseData['error_code']) ? (string) $responseData['error_code'] : null;

        return new self($message, $httpStatusCode, $errorCode, $responseData);
    }

    public static function unauthorized(string $message = 'Ungültiger Plausible API-Key.'): self
    {
    ...
    {
        return new self(
            'Verbindungsfehler zur Plausible API: ' . $message,
            503,
            'CONNECTION_ERROR'
        );
    }
    ...
    {
        return new self(
            'Keine Plausible-Verbindung für diesen Benutzer konfiguriert.',
            401,
            'NO_CONNECTION'
        );
    }
    ...
    {
        return new self(
            'Ungültige Antwort von der Plausible API.' . ($detail ? ' ' . $detail : ''),
            500,
            'INVALID_RESPONSE'
        );
    }
