...
class DeleteTimeTrackingTool implements ToolContract, ToolMetadataContract
{
    use GuardsArguments;

    public function getName(): string
    {
        return 'integrations.easybill.time-tracking.DELETE';
    }

    public function getDescription(): string
    {
        return 'DELETE /time-trackings/{id} — Zeiterfassung löschen.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        if ($guard = $this->guardRequired($arguments, ['time_tracking_id'])) {
            return $guard;
        }

        try {
            $svc = app(EasybillApiService::class)->forConnection($arguments['connection_id'] ?? null);
            $result = $svc->deleteTimeTracking($context->user, (int) $arguments['time_tracking_id']);
