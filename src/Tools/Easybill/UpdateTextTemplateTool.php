...
class UpdateTextTemplateTool implements ToolContract, ToolMetadataContract
{
    use GuardsArguments;

    public function getName(): string
    {
        return 'integrations.easybill.text-template.PUT';
    }

    public function getDescription(): string
    {
        return 'PUT /text-templates/{id} — Text-Vorlage aktualisieren.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        if ($guard = $this->guardRequired($arguments, ['template_id', 'data'])) {
            return $guard;
        }

        try {
            $svc = app(EasybillApiService::class)->forConnection($arguments['connection_id'] ?? null);
            $result = $svc->updateTextTemplate($context->user, (int) $arguments['template_id'], $arguments['data']);
