...
class DeleteCustomerContactTool implements ToolContract, ToolMetadataContract
{
    use GuardsArguments;

    public function getName(): string
    {
        return 'integrations.easybill.customer.contact.DELETE';
    }

    public function getDescription(): string
    {
        return 'DELETE /customers/{customerId}/contacts/{id} — Kontakt löschen.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        if ($guard = $this->guardRequired($arguments, ['customer_id', 'contact_id'])) {
            return $guard;
        }

        try {
            $svc = app(EasybillApiService::class)->forConnection($arguments['connection_id'] ?? null);
            $result = $svc->deleteCustomerContact($context->user, (int) $arguments['customer_id'], (int) $arguments['contact_id']);
