...
class DeleteSerialNumberTool implements ToolContract, ToolMetadataContract
{
    use GuardsArguments;

    public function getName(): string
    {
        return 'integrations.easybill.serial-number.DELETE';
    }

    public function getDescription(): string
    {
        return 'DELETE /serial-numbers/{id} — Seriennummer löschen.';
    }
    ...
    public function getSchema(): array
    {
        return [
          ...
          'properties' => [
            ...
            'serial_id' => [
              'type' => 'integer',
              'description' => 'ID der Seriennummer',
            ],
          ],
          'required' => [
            0 => 'serial_id',
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        if ($guard = $this->guardRequired($arguments, ['serial_id'])) {
            return $guard;
        }

        try {
            $svc = app(EasybillApiService::class)->forConnection($arguments['connection_id'] ?? null);
            $result = $svc->deleteSerialNumber($context->user, (int) $arguments['serial_id']);
