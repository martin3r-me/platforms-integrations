...
class GetWebhookTool implements ToolContract, ToolMetadataContract
{
    use GuardsArguments;

    public function getName(): string
    {
        return 'integrations.easybill.webhook.GET';
    }

    public function getDescription(): string
    {
        return 'GET /webhooks/{id} — Webhook abrufen.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        if ($guard = $this->guardRequired($arguments, ['webhook_id'])) {
            return $guard;
        }

        try {
            $svc = app(EasybillApiService::class)->forConnection($arguments['connection_id'] ?? null);
            $result = $svc->getWebhook($context->user, (int) $arguments['webhook_id']);
