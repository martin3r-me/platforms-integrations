...
class IncomingDocumentPdfTool implements ToolContract, ToolMetadataContract
{
    use GuardsArguments;

    public function getName(): string
    {
        return 'integrations.easybill.incoming-document.pdf';
    }

    public function getDescription(): string
    {
        return 'Komfort: Holt die PDF eines Eingangsbelegs in einem Schritt. Listet intern die Dateien, wählt bevorzugt die PDF (sonst die erste Datei) und liefert sie base64-kodiert zurück (Felder: file, mime, data_base64, size). Read-only.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        try {
            ...
            $result = $svc->getIncomingDocumentPdf($context->user, (string) $arguments['incoming_document_id']);
