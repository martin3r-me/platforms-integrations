...
class DeleteDocumentPaymentTool implements ToolContract, ToolMetadataContract
{
    use GuardsArguments;

    public function getName(): string
    {
        return 'integrations.easybill.document-payment.DELETE';
    }

    public function getDescription(): string
    {
        return 'DELETE /document-payments/{id} — Zahlungseingang löschen.';
    }
    ...
    public function getSchema(): array
    {
        return [
          ...
          'properties' => [
            ...
            'payment_id' => [
              'type' => 'integer',
              'description' => 'ID des Zahlungseingangs',
            ],
          ],
          'required' => [
            0 => 'payment_id',
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        if ($guard = $this->guardRequired($arguments, ['payment_id'])) {
            return $guard;
        }

        try {
            $svc = app(EasybillApiService::class)->forConnection($arguments['connection_id'] ?? null);
            $result = $svc->deleteDocumentPayment($context->user, (int) $arguments['payment_id']);
