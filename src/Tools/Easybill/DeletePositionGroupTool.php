...
class DeletePositionGroupTool implements ToolContract, ToolMetadataContract
{
    use GuardsArguments;

    public function getName(): string
    {
        return 'integrations.easybill.position-group.DELETE';
    }

    public function getDescription(): string
    {
        return 'DELETE /position-groups/{id} — Artikelgruppe löschen.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        if ($guard = $this->guardRequired($arguments, ['group_id'])) {
            return $guard;
        }

        try {
            $svc = app(EasybillApiService::class)->forConnection($arguments['connection_id'] ?? null);
            $result = $svc->deletePositionGroup($context->user, (int) $arguments['group_id']);
