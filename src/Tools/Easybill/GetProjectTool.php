...
class GetProjectTool implements ToolContract, ToolMetadataContract
{
    ...
    {
        return 'integrations.easybill.project.GET';
    }

    public function getDescription(): string
    {
        return 'GET /projects/{id} — Projekt abrufen.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        if ($guard = $this->guardRequired($arguments, ['project_id'])) {
            return $guard;
        }

        try {
            $svc = app(EasybillApiService::class)->forConnection($arguments['connection_id'] ?? null);
            $result = $svc->getProject($context->user, (int) $arguments['project_id']);
