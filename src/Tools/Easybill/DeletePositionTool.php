...
class DeletePositionTool implements ToolContract, ToolMetadataContract
{
    ...
    {
        return 'integrations.easybill.position.DELETE';
    }

    public function getDescription(): string
    {
        return 'DELETE /positions/{id} — Artikel löschen.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        if ($guard = $this->guardRequired($arguments, ['position_id'])) {
            return $guard;
        }

        try {
            $svc = app(EasybillApiService::class)->forConnection($arguments['connection_id'] ?? null);
            $result = $svc->deletePosition($context->user, (int) $arguments['position_id']);
