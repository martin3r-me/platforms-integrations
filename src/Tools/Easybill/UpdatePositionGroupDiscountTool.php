...
class UpdatePositionGroupDiscountTool implements ToolContract, ToolMetadataContract
{
    use GuardsArguments;

    public function getName(): string
    {
        return 'integrations.easybill.discount.position-group.PUT';
    }

    public function getDescription(): string
    {
        return 'PUT /discounts/position-group/{id} — Gruppen-Rabatt aktualisieren.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        if ($guard = $this->guardRequired($arguments, ['discount_id', 'data'])) {
            return $guard;
        }

        try {
            $svc = app(EasybillApiService::class)->forConnection($arguments['connection_id'] ?? null);
            $result = $svc->updatePositionGroupDiscount($context->user, (int) $arguments['discount_id'], $arguments['data']);
