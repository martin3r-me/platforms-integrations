...
class GetSepaPaymentTool implements ToolContract, ToolMetadataContract
{
    use GuardsArguments;

    public function getName(): string
    {
        return 'integrations.easybill.sepa-payment.GET';
    }

    public function getDescription(): string
    {
        return 'GET /sepa-payments/{id} — SEPA-Zahlung abrufen.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        if ($guard = $this->guardRequired($arguments, ['sepa_payment_id'])) {
            return $guard;
        }

        try {
            $svc = app(EasybillApiService::class)->forConnection($arguments['connection_id'] ?? null);
            $result = $svc->getSepaPayment($context->user, (int) $arguments['sepa_payment_id']);
