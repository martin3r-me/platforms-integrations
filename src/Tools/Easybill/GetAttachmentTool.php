...
class GetAttachmentTool implements ToolContract, ToolMetadataContract
{
    use GuardsArguments;

    public function getName(): string
    {
        return 'integrations.easybill.attachment.GET';
    }

    public function getDescription(): string
    {
        return 'GET /attachments/{id} — Anhang-Metadaten abrufen.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        if ($guard = $this->guardRequired($arguments, ['attachment_id'])) {
            return $guard;
        }

        try {
            $svc = app(EasybillApiService::class)->forConnection($arguments['connection_id'] ?? null);
            $result = $svc->getAttachment($context->user, (int) $arguments['attachment_id']);
