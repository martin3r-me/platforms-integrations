...
class GetCreditNoteTool implements ToolContract, ToolMetadataContract
{
    public function getName(): string
    {
        return 'integrations.lexware.credit_note.GET';
    }

    public function getDescription(): string
    {
        return 'GET /credit-notes/{id} - Ruft eine einzelne Lexware-Gutschrift ab. id (string, UUID).';
    }
    ...
    public function getSchema(): array
    {
        return [
            ...
            'properties' => [
                ...
                'id' => ['type' => 'string', 'description' => 'UUID der Gutschrift'],
            ],
            'required' => ['id'],
        ];
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        if (empty($arguments['id'])) {
            return ToolResult::error('VALIDATION_ERROR', 'Gutschrift-ID ist erforderlich.');
        }

        try {
            $service = app(LexwareApiService::class)->forConnection($arguments['connection_id'] ?? null);
            $result = $service->getCreditNote($context->user, $arguments['id']);
    ...
    public function getMetadata(): array
    {
        return [
            'category' => 'query',
            'tags' => ['lexware', 'credit_notes', 'detail'],
            'read_only' => true,
            'requires_auth' => true,
            'risk_level' => 'safe',
        ];
