...
/**
 * DedeFleet POST /User/Notify — Sends a notification message to a user, a group, or all administrators.
 * Auto-generiert aus der v2-Swagger-Spec; delegiert an DedefleetApiService::call().
 */
class UserNotifyTool implements ToolContract, ToolMetadataContract
{
    public function getName(): string
    {
        return 'integrations.dedefleet.user.notify.POST';
    }

    public function getDescription(): string
    {
        return 'Sends a notification message to a user, a group, or all administrators.

REQUEST-Felder (`data`):
- message: string — Message text
- forAdmins: boolean — This message should be sent to all admins.
- forUser: string — Username as shown in DeDeFleet.
- forGroup: string — Usergroup as shown in DeDeFleet.
...
Vollständige Feld-/Response-Details: https://ortung.dedefleet.de/swagger (Spec /swagger/data/api/2).';
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        try {
            ...
            $result = $svc->call($context->user, 'POST', '/User/Notify', $payload);
