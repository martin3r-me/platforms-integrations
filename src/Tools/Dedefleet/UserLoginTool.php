...
/**
 * DedeFleet POST /User/Login — Authenticates a user and returns an API token for subsequent requests.
 * Auto-generiert aus der v2-Swagger-Spec; delegiert an DedefleetApiService::call().
 */
class UserLoginTool implements ToolContract, ToolMetadataContract
{
    public function getName(): string
    {
        return 'integrations.dedefleet.user.login.POST';
    }

    public function getDescription(): string
    {
        return 'Authenticates a user and returns an API token for subsequent requests.
...
Vollständige Feld-/Response-Details: https://ortung.dedefleet.de/swagger (Spec /swagger/data/api/2).';
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        try {
            ...
            $result = $svc->call($context->user, 'POST', '/User/Login', $payload);
