...
/**
 * DedeFleet POST /Order/GetStatusBulk — Basically the same as "Order/GetStatus" but for bulk operations
 * Auto-generiert aus der v2-Swagger-Spec; delegiert an DedefleetApiService::call().
 */
class OrderGetStatusBulkTool implements ToolContract, ToolMetadataContract
{
    public function getName(): string
    {
        return 'integrations.dedefleet.order.get-status-bulk.POST';
    }

    public function getDescription(): string
    {
        return 'Basically the same as "Order/GetStatus" but for bulk operations

REQUEST-Felder (`data`):
- orderGuids: array<string> — List of order GUIDs whose status should be returned.
...
Vollständige Feld-/Response-Details: https://ortung.dedefleet.de/swagger (Spec /swagger/data/api/2).';
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        ...
        try {
            ...
            $result = $svc->call($context->user, 'POST', '/Order/GetStatusBulk', $payload);
