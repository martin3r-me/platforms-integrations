...
class NectaApiV1Service
{
    ...
    {
        $paths = array_values(array_filter(array_map(static fn ($f) => trim((string) $f), $fields), static fn ($f) => $f !== ''));
        if (!$paths || !is_array($response)) {
            return $response;
        }

        if (array_is_list($response)) {
            return array_map(static fn ($it) => is_array($it) ? self::pickPaths($it, $paths) : $it, $response);
        }

        foreach (['result', 'items', 'data', 'content'] as $key) {
            if (isset($response[$key]) && is_array($response[$key]) && array_is_list($response[$key])) {
                $response[$key] = array_map(static fn ($it) => is_array($it) ? self::pickPaths($it, $paths) : $it, $response[$key]);
                return $response;
            }
        }
     ...
     * @return array<string, mixed>
     */
    protected static function pickPaths(array $item, array $paths): array
    {
        $out = [];
     ...
     */
    protected static function setPath(array &$target, array $segs, mixed $value): void
    {
        $ref = &$target;
